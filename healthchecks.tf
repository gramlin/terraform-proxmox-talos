...
KUBECONFIG
      ...
      while true; do
        ...
        if command -v talosctl >/dev/null 2>&1; then
          if ! talosctl --talosconfig "$talos_config" --endpoints "$controllers_csv" get members --nodes "$controllers_csv"; then
            echo "⚠️  Talos: väntar på noder..."
          fi
          if talosctl --talosconfig "$talos_config" --endpoints "$controllers_csv" health --wait-timeout 10s --control-plane-nodes "$controllers_csv" --worker-nodes "$workers_csv"; then
      ...
      if command -v talosctl >/dev/null 2>&1; then
        ...
        talosctl --talosconfig "$talos_config" --endpoints "$controllers_csv" health --wait-timeout 20m --control-plane-nodes "$controllers_csv" --worker-nodes "$workers_csv"
        echo "🟢 Talos: medlemmar"
        talosctl --talosconfig "$talos_config" --endpoints "$controllers_csv" get members --nodes "$controllers_csv"
      else
